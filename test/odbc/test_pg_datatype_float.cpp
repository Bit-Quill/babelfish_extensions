...
const string COL_NAMES[NUM_COLS] = {"pk", "float_max","float_min"};
const int COL_LENGTH[NUM_COLS]={10,308,308};
const int COL_SCALE[NUM_COLS] = {0, 308, 308};
...
TEST_F(PSQL_DataTypes_Float, ColAttributes) {
  ...
  const int Lenghth_expected[NUM_COLS]={8,17,17};
  const int PREICSION_EXPECTED=0;
  const int SCALE_EXPECTED=0;
  const int COL_LENGTH_EXPECTED[NUM_COLS]={24,24,24};
...
TEST_F(PSQL_DataTypes_Float, Insertion_Success) {
  ...
  vector<vector<string>> inserted_values = {
    {"1", "0", "0" }, // smallest numbers
    {"2", float_15,float_383}, // max values
    {"3", "-0.694", "0.4347509234", }, // random regular values
    {"4", "NULL", "NULL"} // NULL values
  };
...
TEST_F(PSQL_DataTypes_Float, Update_Success) {
  ...
  vector<vector<string>> updated_values = {
    // standard values
    {PK_VAL, "1e50", "-1e50"}, // max values
    {PK_VAL, "0", "0"},
    {PK_VAL,float_15,float_383} // min values
...
TEST_F(PSQL_DataTypes_Float, Update_Fail) {
  ...
  vector<vector<string>> updated_values = {
    {"1", "1e400","0"} // first col exceeds by 1 digit
  };
